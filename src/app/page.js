  ...
  useEffect(() => {
    if (swiperInstance && activeCategory) {
      ...
      const isCurrentlyOnTarget =
        swiperInstance.realIndex % filterOptions.length === index;

      if (
        index !== -1 &&
        !isCurrentlyOnTarget &&
        !swiperInstance.animating &&
        !swiperInstance?.touchEventsData?.isTouched
      ) {
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <section className={styles.hero}>
        <div className="container">
          <h1 className={`${styles.title} animate-fade`}>
            Modern <span className={styles.gradientText}>Catalog</span>{" "}
            Experience
          </h1>
          <p className={`${styles.subtitle} animate-fade`}>
            Explore cutting-edge technology and precision engineering across
                  ...
                  {(filterOptions.length < 6
                    ...
                    >
                      {({ isActive, progress }) => {
                        ...
                        const startR = 138,
                          startG = 43,
                          startB = 226; // var(--primary) (#8a2be2)
                        const r = Math.round(
                          startR + (targetR - startR) * influence,
                        );
                        const g = Math.round(
                          startG + (targetG - startG) * influence,
                        );
                        const b = Math.round(
                          startB + (targetB - startB) * influence,
                        );
      ...
      <footer className={styles.footer}>
        <div className="container">
          <p>
            &copy; 2026 Multi-Category-Catalog. Dynamic Frontend Assignment.
          </p>
        </div>
      </footer>
    </div>
